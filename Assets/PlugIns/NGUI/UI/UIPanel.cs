...
public class UIPanel : UIRect
{
    ...
    private bool mRealRebuild = false;
	bool mRebuild
    {
        get { return mRealRebuild; }
        set 
        { 
            mRealRebuild = value;
            // Debug.Log("---Panel Need to rebuild---");
        }
    }
	bool mResized = false;
	...
	void UpdateSelf ()
	{
		...
		if (mRebuild)
		{
            ...
            // Debug.Log("Rebuild panel name " + gameObject.name);
		}
		else
		{
			for (int i = 0; i < drawCalls.size; )
			{
				...
				if (dc.isDirty && !FillDrawCall(dc))
				{
                    ...
                    // Debug.Log("remove draw call " + dc.name);
					continue;
				}
				++i;
			}
